...
const Partners: React.FC = () => {
    ...
    return (
        <section className="py-12 md:py-20 bg-[#073215] overflow-hidden transition-colors duration-300">
            <div className="container mx-auto px-4 md:px-6 mb-8 md:mb-12">
                <h2 className="text-2xl md:text-3xl font-bold text-white text-center md:text-left">{t('partners.title')}</h2>
            </div>
            ...
            <div className="relative w-full flex flex-col gap-4 md:gap-8">
                ...
                <div className="flex w-max animate-scroll-right group gap-4 md:gap-8">
                    {topRowLogos.map((logo, index) => (
                        <div key={`r1-${index}`} className="w-32 h-20 md:w-48 md:h-28 bg-white/90 border border-transparent shadow-sm rounded-lg flex items-center justify-center p-4 hover:border-fastway-orange hover:shadow-md transition-all">
                ...
                <div className="flex w-max animate-scroll-left group gap-4 md:gap-8">
                    {bottomRowLogos.map((logo, index) => (
                        <div key={`r2-${index}`} className="w-32 h-20 md:w-48 md:h-28 bg-white/90 border border-transparent shadow-sm rounded-lg flex items-center justify-center p-4 hover:border-fastway-orange hover:shadow-md transition-all">
                            <img src={logo} alt="Partner" className="max-w-full max-h-full object-contain" />
                        </div>
                    ))}
                </div>
