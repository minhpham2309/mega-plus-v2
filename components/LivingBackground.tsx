...
const LivingBackground: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-[#0A3A20] opacity-90 z-10 mix-blend-color"></div>
      
      {/* Blob 1: Orange Warmth */}
      <div 
        ref={el => { blobRefs.current[0] = el; }}
        className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full bg-[#F59E0B] opacity-10 blur-[100px] mix-blend-screen"
      />

      {/* Blob 2: Deep Green */}
      <div 
        ref={el => { blobRefs.current[1] = el; }}
        className="absolute bottom-1/4 right-1/4 w-[600px] h-[600px] rounded-full bg-[#16A34A] opacity-10 blur-[120px] mix-blend-screen"
      />

      {/* Blob 3: Accent Light */}
      <div 
        ref={el => { blobRefs.current[2] = el; }}
        className="absolute top-1/2 left-1/2 w-[300px] h-[300px] rounded-full bg-[#F59E0B] opacity-5 blur-[80px] mix-blend-screen"
      />
    </div>
  );
};
